...
function enhancementBatchRegistry() {
  return window.__localLLMEnhancementBatches instanceof Map ? window.__localLLMEnhancementBatches : null;
}

function enhancementBatchActive() {
  return (enhancementBatchRegistry()?.size || 0) > 0;
}

window.addEventListener("local-llm-enhance-batch-state", () => {
  if (modal?.isConnected && !enhancementBatchActive()) {
    const note = modal.querySelector("#llm-save-note");
    if (note?.textContent === "Applied • after current batch") {
      note.textContent = status.restart_required ? "Applied • next request will reload model" : "Applied";
    }
  }
  updateLiveUI();
});
...
function renderTuner() {
  ...
  const msg=modal.querySelector("#llm-tuner-message"); if(msg)msg.textContent=t.error ? t.error : (t.message||"Ready to benchmark the current server configuration.");
  ...
  if(run){run.disabled=running||saving||!ready;run.title=saving?"Applying configuration changes":(!ready?"Start the Local LLM service before benchmarking":"Benchmark the current configuration");}
...
async function startTuner() {
  if(!(await saveConfig())){const n=modal?.querySelector("#llm-tuner-message");if(n)n.textContent="Could not apply the current configuration before benchmarking.";return;}
  const body={
...
async function saveTunerPreset() {
  ...
  const name=window.prompt("Name this complete settings preset:",`${modelName} Tuned`);
  if(!name)return;
  try{
    const settings={...collectConfig(),...(rec.settings||{})};
    const d=await fetchJSON("/local_llm_server/settings_presets",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name,settings})});
    catalog=d.catalog||catalog;
    refreshSettingsPresetTab(d.saved?.name||name);
    refreshAllDefaultServiceGenerateNodes();
    const n=modal.querySelector("#llm-tuner-message");if(n)n.textContent=`Saved complete settings preset: ${d.saved?.name||name}`;
  }catch(e){const n=modal.querySelector("#llm-tuner-message");if(n)n.textContent=`Preset save failed: ${e.message}`;}
}


function settingsPresetSection(){return catalog?.settings_presets||{names:["Custom"],deletable_names:[],presets:{}};}
const COMPLETE_SETTINGS_PRESET_FIELDS = [
  "model","vision_model","model_preset","thinking_mode","reasoning_effort","preserve_thinking",
  "temperature","top_p","top_k","min_p","repeat_penalty","presence_penalty","frequency_penalty","max_tokens",
  "vision_max_images","vision_max_frames","vision_max_edge",
  "context_size","kv_cache_k","kv_cache_v","kv_cache_location","gpu_layers","flash_attention",
  "prompt_batch_size","memory_batch_size","use_mmap","use_mlock","prompt_cache_mode",
  "speculative_mode","ngram_pred_tokens","ngram_size","ngram_mode","ngram_min_hits",
  "ngram_max_entries_per_key","ngram_sync_check_tokens","mtp_draft_tokens","mtp_p_min",
  "split_mode","main_gpu","tensor_split","vram_policy",
];
function completeSettingsPresetMatchesCurrent(settings,current=null){
  if(!settings||typeof settings!=="object"||!modal?.isConnected)return false;
  const now=current||collectConfig();
  // A complete preset is considered active only when every runtime field that
  // exists in the current modal is also present in the preset and equal. This
  // intentionally makes older/partial presets display as Custom rather than
  // claiming a match while newer settings differ.
  for(const key of COMPLETE_SETTINGS_PRESET_FIELDS){
    if(!(key in now))continue;
    if(!(key in settings))return false;
    const a=now[key], b=settings[key];
    if(typeof a==="number" || typeof b==="number"){
      if(!Number.isFinite(Number(a)) || !Number.isFinite(Number(b)) || Number(a)!==Number(b))return false;
...
function currentSettingsPresetName(preferred=null){
  const section=settingsPresetSection();
  const names=(section.names||[]).filter(x=>x!=="Custom" && section.presets?.[x]);
  const current=collectConfig();
  if(preferred && names.includes(preferred) && completeSettingsPresetMatchesCurrent(section.presets[preferred],current))return preferred;
  return names.find(name=>completeSettingsPresetMatchesCurrent(section.presets[name],current)) || "Custom";
}
function settingsPresetSummary(settings){
  if(!settings||!Object.keys(settings).length)return "Select a saved preset to inspect it.";
  const rows=[];
  const add=(label,key)=>{if(settings[key]!==undefined&&settings[key]!==null&&settings[key]!=="")rows.push(`${label}: ${settings[key]}`);};
  add("Model","model");add("Vision / mmproj","vision_model");add("Model preset","model_preset");add("Thinking","thinking_mode");add("Reasoning effort","reasoning_effort");
  add("Temperature","temperature");add("Top P","top_p");add("Top K","top_k");add("Max tokens","max_tokens");
  add("Context","context_size");add("KV K","kv_cache_k");add("KV V","kv_cache_v");add("KV location","kv_cache_location");add("GPU layers","gpu_layers");
  add("Flash Attention","flash_attention");add("Prompt batch","prompt_batch_size");add("Micro batch","memory_batch_size");add("Speculative","speculative_mode");add("VRAM policy","vram_policy");
  return rows.join("\n")||"Preset contains supported advanced settings.";
}
function refreshSettingsPresetTab(preferred=null){
  if(!modal?.isConnected)return;
  const section=settingsPresetSection();
  const select=modal.querySelector("#llm-settings-preset-select");
  if(!select)return;
  const names=(section.names||[]).filter(x=>x!=="Custom" && section.presets?.[x]);
  // With no explicit browsing choice, reflect the configuration that is
  // actually in the modal. Never fall back to the first saved preset.
  const selected = preferred!==null
    ? (preferred==="Custom" || names.includes(preferred) ? preferred : currentSettingsPresetName())
    : currentSettingsPresetName();
  select.innerHTML=selectOptions(["Custom",...names],selected);
  select.disabled=false;
  const current=select.value;
  const summary=modal.querySelector("#llm-settings-preset-summary");
  if(summary){
    if(current==="Custom") summary.textContent=names.length
      ? "Current settings do not exactly match a saved preset."
      : "No complete settings presets have been saved yet. Current settings are Custom.";
    else summary.textContent=settingsPresetSummary(section.presets?.[current]);
  }
  const del=modal.querySelector('[data-action="preset-delete"]');if(del)del.disabled=current==="Custom"||!(section.deletable_names||[]).includes(current);
  const load=modal.querySelector('[data-action="preset-load"]');
  if(load){
    const selectedPreset=current!=="Custom" ? section.presets?.[current] : null;
    const selectedIsActive=!!selectedPreset && completeSettingsPresetMatchesCurrent(selectedPreset,collectConfig());
    load.disabled=current==="Custom"||!selectedPreset||selectedIsActive;
    load.title=selectedIsActive ? "This preset is already active." : (selectedPreset ? `Load ${current}` : "Select a saved preset to load.");
  }
  // An explicit user selection is a browsing/load choice. Keep it selected
  // until settings change or the tab is reopened, at which point the current
  // configuration is resolved again.
  select.onchange=()=>refreshSettingsPresetTab(select.value);
}
function syncSettingsPresetSelection(){
  if(!modal?.isConnected)return;
  const select=modal.querySelector("#llm-settings-preset-select");
  if(!select)return;
  const matched=currentSettingsPresetName(select.value);
  if(select.value!==matched)refreshSettingsPresetTab(matched);
  else {
    // Keep the summary/buttons correct when settings changed but the same
    // preset still matches (for example, a server-only preference changed).
    refreshSettingsPresetTab(matched);
  }
}
async function applyCompleteSettingsToModal(settings){
  if(!settings)return;
  if(settings.model!==undefined){setConfigField("model",settings.model);config.model=settings.model;await updateModelPresetChoices();}
  for(const [k,v] of Object.entries(settings))setConfigField(k,v);
  if(settings.context_size!==undefined)updateContextChoices(currentModelInfo?.context_sizes||catalog.context_sizes||[],currentModelInfo?.native_context);
  applySpeculativeUI(currentModelInfo||{capabilities:status?.capabilities,speculative_support:status?.speculative_support,speculative:status?.speculative});
  setDirty(true);scheduleVRAMEstimate(60);
}
async function loadSelectedSettingsPreset(){
  const sel=modal?.querySelector("#llm-settings-preset-select");const name=sel?.value;const settings=settingsPresetSection().presets?.[name];
  if(!settings)return;await applyCompleteSettingsToModal(settings);const n=modal?.querySelector("#llm-save-note");if(n)n.textContent=`Loaded preset: ${name} — applying automatically…`;
}
async function saveCurrentSettingsPreset(){
  const modelName=String(config?.model||"Local LLM").split(/[\\/]/).pop().replace(/\.gguf$/i,"");
  const name=window.prompt("Name this complete settings preset:",modelName?`${modelName} Settings`:"");if(!name?.trim())return;
  try{const d=await fetchJSON("/local_llm_server/settings_presets",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:name.trim(),settings:collectConfig()})});catalog=d.catalog||catalog;refreshSettingsPresetTab(d.saved?.name||name.trim());refreshAllDefaultServiceGenerateNodes();toast("success","Local LLM Preset",`Saved ${d.saved?.name||name.trim()}`);}catch(e){toast("error","Local LLM Preset",e.message);}
}
async function deleteSelectedSettingsPreset(){
  const sel=modal?.querySelector("#llm-settings-preset-select");const name=sel?.value;if(!name)return;if(!window.confirm(`Delete complete settings preset “${name}”?`))return;
  try{const d=await fetchJSON("/local_llm_server/settings_presets",{method:"DELETE",headers:{"Content-Type":"application/json"},body:JSON.stringify({name})});catalog=d.catalog||catalog;refreshSettingsPresetTab();refreshAllDefaultServiceGenerateNodes();toast("success","Local LLM Preset",`Deleted ${name}`);}catch(e){toast("error","Local LLM Preset",e.message);}
}
...
function renderModal() {
  ...
  modal.innerHTML = `
    <div id="local-llm-modal">
      ...
      <div class="llm-tabs">
        <button class="llm-tab active" data-tab="server">Server</button><button class="llm-tab" data-tab="presets">Presets</button><button class="llm-tab" data-tab="model">Model</button><button class="llm-tab" data-tab="memory">Memory</button><button class="llm-tab" data-tab="tuner">Tuner</button><button class="llm-tab" data-tab="api">API</button><button class="llm-tab" data-tab="logs">Logs</button>
      </div>
      <div class="llm-body">
        ...
        <div class="llm-pane" data-pane="presets">
          <div class="llm-card"><h3>Complete Settings Presets</h3>
            <div class="llm-grid">
              <label>Preset</label><select id="llm-settings-preset-select"></select>
            </div>
            <div class="llm-actions" style="margin-top:11px">
              <button class="llm-btn primary" data-action="preset-load">Load Preset</button>
              <button class="llm-btn" data-action="preset-save">Save Current</button>
              <button class="llm-btn danger" data-action="preset-delete">Delete</button>
            </div>
            <div class="llm-note">Complete presets store the LLM runtime configuration: model, vision projector, model behavior and sampling, plus memory/KV/offload/speculative settings. Server administration such as API keys, startup mode, logging, and interface preferences is not included.</div>
          </div>
          <div class="llm-card"><h3>Preset Summary</h3><div id="llm-settings-preset-summary" class="llm-note" style="white-space:pre-wrap;line-height:1.45">Select a preset to inspect it.</div></div>
        </div>
        ...
        <div class="llm-pane" data-pane="tuner">
          ...
          <div class="llm-card"><h3>Progress</h3>
            ...
            <div id="llm-tuner-message" class="llm-muted" style="margin-top:5px">Ready to benchmark the current server configuration.</div>
          </div>
          <div class="llm-card" id="llm-tuner-recommendation"><h3>Recommendation</h3>
            ...
            <div class="llm-actions" style="margin-top:11px"><button class="llm-btn" data-action="tuner-use" disabled>Use Recommendation</button><button class="llm-btn" data-action="tuner-save-preset" disabled>Save as Complete Preset</button></div>
          </div>
      ...
      <div class="llm-footer"><span id="llm-save-note" class="llm-muted" style="margin-right:auto"></span><button class="llm-btn warn" data-action="reload">Reload Model</button></div>
    </div>`;
...
function renderVRAMEstimate(data=vramEstimate) {
  ...
  if(cap){
    ...
    if(data.measured_bytes>0) parts.push(data.measured_applies ? `Measured value is valid for the current configuration (${data.measured_source || "prior load"}).` : "* Measured value is from the currently loaded/previous configuration, not these newly selected settings.");
...
function runtimeNote() {
  ...
  if (enhancementBatchActive()) {
    if (status.restart_required) return `<span class="llm-warn">Enhance batch active. Changes are saved, but this batch stays pinned to the model/settings it started with. New settings apply after the batch.</span>`;
    return `<span class="llm-muted">Enhance batch active. Modal changes are saved for the next request; the running batch keeps its starting model/settings.</span>`;
  }
  if (status.restart_required) return `<span class="llm-warn">Settings updated. The new model/load settings will be applied automatically on the next request.</span>`;
...
function updateLiveUI() {
  ...
  const family=modal.querySelector("#llm-detected-family");
  if(family) family.textContent=currentModelInfo?.family || (modelInfoLoading ? "Detecting…" : (status.family || "unknown"));
...
async function updateModelPresetChoices() {
  ...
  const family = modal?.querySelector("#llm-detected-family");
  if (family) family.textContent = "Detecting…";
  try {
    const info = await fetchJSON(`/local_llm_server/model_info?model=${encodeURIComponent(model)}`);
    if ((modal?.querySelector("#llm-model-select")?.value || config?.model) !== requestedModel) return;
    ...
    const fam = modal?.querySelector("#llm-detected-family");
    if (fam) fam.textContent = "unknown";
    ...
    if ((modal?.querySelector("#llm-model-select")?.value || config?.model) === requestedModel) modelInfoLoading = false;
  }
}
...
function setDirty(value=true) {
  ...
  dirty = true;
  configEditVersion += 1;
  // Complete Settings Preset is a reflection of the current runtime config,
  // not a sticky UI choice. Any runtime edit immediately resolves it to the
  // matching preset or Custom.
  syncSettingsPresetSelection();
  const note = modal?.querySelector("#llm-save-note");
  if (note) note.textContent = "Applying…";
  updateActionButtons();
  // Persist on the same UI turn. Multiple rapid edits are serialized and the
  // newest modal state is written again if an older PUT is already in flight.
  queueMicrotask(() => { void saveConfig(); });
}
...
function updateActionButtons() {
  ...
  if (start) {
    start.disabled = busy || serviceActive || saving;
    start.title = saving ? "Applying configuration changes" : (serviceActive ? (yielded ? "Service is suspended and will fast-reload on the next request" : "Model is already running") : "Start the current server configuration");
  }
  if (suspend) {
  ...
  if (reload) {
    ...
    reload.disabled = saving || busy || !serviceActive || batchActive;
    reload.title = !serviceActive
      ? "Reload is available only while the service is started"
      : (batchActive
        ? "Enhance batch is pinned to its starting model/settings; reload becomes available when the batch finishes"
        : (saving ? "Applying configuration changes" : (busy ? "Wait for the current operation to finish" : "Perform a deliberate full reload using the current settings")));
  }
}

async function saveConfig() {
  // If an autosave loop is already running, callers such as Reload/Tuner/Close
  // wait for that complete loop, including any edits made while its PUT was in
  // flight. This keeps server actions from racing a partially-applied config.
  if (saving) return savePromise || true;
  ...
  savePromise = (async () => {
    let ok = true;
    while (dirty && modal?.isConnected) {
      const saveVersion = configEditVersion;
      const payload = collectConfig();
      // Any edit that happens while this request is in flight sets dirty=true
      // again, causing the loop to persist the newest full modal state next.
      dirty = false;
      if (note) note.textContent = "Applying…";
      try {
        const data = await fetchJSON("/local_llm_server/config", {method:"PUT", headers:{"Content-Type":"application/json"}, body:JSON.stringify(payload)});
        config = data.config;
        status = data.status;
        ensureFloatingStatus();

        // Only normalize controls from the response if no newer edit exists.
        if (saveVersion === configEditVersion && !dirty) {
          setConfigField("api_key", config.api_key || "");
          setConfigField("context_size", config.context_size);
          syncSettingsPresetSelection();
          if (note) note.textContent = enhancementBatchActive()
            ? "Applied • after current batch"
            : (status.restart_required ? "Applied • next request will reload model" : "Applied");
        } else if (note) {
          note.textContent = "Applying latest changes…";
        }
        updateLiveUI();
        refreshAllDefaultServiceGenerateNodes();
      } catch(e) {
  ...
  if (ok && !dirty) {
    setTimeout(()=>{
      if(modal?.isConnected && !dirty && !saving){
        const n=modal.querySelector("#llm-save-note");
        if(n?.textContent==="Applied") n.textContent="";
      }
    },1400);
  }
  return ok;
...
async function doAction(action) {
  if ((action === "start" || action === "reload") && !(await saveConfig())) return;
  try {
...
function bindModal() {
  ...
  for(const tab of modal.querySelectorAll(".llm-tab")) tab.onclick=()=>{
    ...
    if(tab.dataset.tab==="presets") refreshSettingsPresetTab();
  ...
  for(const el of modal.querySelectorAll(".llm-config[data-key]")) el.addEventListener("change",()=>{
    const k=el.dataset.key;
    if(k==="model" || k==="model_preset") return;
  ...
  for(const b of modal.querySelectorAll("[data-action]")) b.onclick=async()=>{
    ...
    if(a==="preset-load") return loadSelectedSettingsPreset();
    if(a==="preset-save") return saveCurrentSettingsPreset();
    if(a==="preset-delete") return deleteSelectedSettingsPreset();
    ...
    if(a==="regen-key"){if(!(await saveConfig()))return;const d=await fetchJSON("/local_llm_server/api_key/regenerate",{method:"POST"});config.api_key=d.api_key;setConfigField("api_key",d.api_key);dirty=false;updateActionButtons();return;}
  };
}

async function closeModal(){
  const closingModal = modal;
  if(closingModal && (dirty || saving)) await saveConfig();
  if(modal === closingModal && modal){modal.remove();modal=null;}
  if(logsTimer){clearInterval(logsTimer);logsTimer=null;}
  if(vramEstimateTimer){clearTimeout(vramEstimateTimer);vramEstimateTimer=null;}
}
...
function installCatalogChoices(node, data) {
  ...
  const settingsSel = nodeWidget(node, "settings_preset");
  ...
  setPresetChoices(settingsSel, data?.settings?.names || [], ["Current Server", "Custom"]);
...
function applyCompleteSettingsToNode(node, settings) {
  if(!node || !settings)return;
  node.__localLLMApplyingPreset=true;
  try{
    for(const [key,value] of Object.entries(settings)){
      const w=nodeWidget(node,key);if(!w)continue;w.value=value;try{w.callback?.(value);}catch(_){}
    }
    const mode=nodeWidget(node,"sampling_mode");if(mode)mode.value="Custom";
  }finally{node.__localLLMApplyingPreset=false;}
  persistServiceNodeUIState(node);scheduleServiceNodePanelSync(node);markServiceNodeChanged(node);
}
async function applySettingsPresetSelection(node, selection, suppliedCatalog=null){
  const data=suppliedCatalog||await fetchNodePresetCatalog();installCatalogChoices(node,data);
  const name=String(selection||"Current Server");
  if(name==="Custom")return;
  if(name==="Current Server"){
    const current=await fetchJSON("/local_llm_server/config");
    applyCompleteSettingsToNode(node,current);
    return;
  }
  const settings=data?.settings?.presets?.[name];if(settings)applyCompleteSettingsToNode(node,settings);
}

async function applySettingsModelPreset(node, selection, suppliedCatalog=null){
  let name=String(selection||"Custom");if(name==="Custom")return;
  const data=suppliedCatalog||node.__localLLMPresetCatalog||await fetchNodePresetCatalog();
  if(name==="Auto (Detected)"){
    try{const model=String(nodeWidget(node,"model")?.value||"");const info=await fetchJSON(`/local_llm_server/model_info?model=${encodeURIComponent(model)}`);name=info.recommended_preset||"Generic Chat";}catch(_){name="Generic Chat";}
  }
  const values=data?.model_runtime_presets?.[name];if(!values)return;
  node.__localLLMApplyingPreset=true;
  try{for(const [key,value] of Object.entries(values)){const w=nodeWidget(node,key);if(w){w.value=value;try{w.callback?.(value);}catch(_){}}}}finally{node.__localLLMApplyingPreset=false;}
  persistServiceNodeUIState(node);scheduleServiceNodePanelSync(node);markServiceNodeChanged(node);
}
...
function installPresetCallbacks(node) {
  ...
  for (const [widgetName, kind] of selectors) {
    const w = nodeWidget(node, widgetName);
    if (!w) continue;
    if (widgetName === "settings_preset") w.label = "Settings Preset";
    else if (widgetName === "sampling_mode") w.label = "Sampler Preset";
    ...
    w.callback = function(value, ...args) {
      ...
      if (!node.__localLLMApplyingPreset) {
        let promise;
        if(kind === "settings") promise=applySettingsPresetSelection(node,value);
        else promise=applyPresetSelection(node,kind,value);
        promise.then(()=>scheduleServiceNodePanelSync(node)).catch((e)=>console.warn("[Local LLM] preset load failed",e));
      }
      scheduleServiceNodePanelSync(node);
      return result;
    };
  ...
  if(node?.comfyClass === "LocalLLMSettings"){
    const mp=nodeWidget(node,"model_preset");
    if(mp && !mp.__localLLMModelPresetWrapped){mp.__localLLMModelPresetWrapped=true;const old=mp.callback;mp.callback=function(value,...args){const result=old?.call(this,value,...args);if(!node.__localLLMApplyingPreset)applySettingsModelPreset(node,value).catch(e=>console.warn("[Local LLM] model preset load failed",e));scheduleServiceNodePanelSync(node);return result;};}
  }
  const owned = new Set([...REQUEST_PRESET_FIELDS]);
  if(node?.comfyClass === "LocalLLMSettings") for(const k of SETTINGS_ALL_PRESET_OWNED) owned.add(k);
  for (const key of owned) {
    const w = nodeWidget(node, key); if (!w) continue;
    const old = w.callback;
    w.callback = function(value, ...args) {
      const result = old?.call(this, value, ...args);
      if (!node.__localLLMApplyingPreset) {
        const sampler=nodeWidget(node,"sampling_mode");if(sampler)sampler.value="Custom";
        if(node?.comfyClass === "LocalLLMSettings"){
          const p=nodeWidget(node,"settings_preset");if(p&&p.value!=="Custom")p.value="Custom";
          if(REQUEST_PRESET_FIELDS.includes(key) || ["thinking_mode","reasoning_effort","preserve_thinking"].includes(key)){const mp=nodeWidget(node,"model_preset");if(mp&&mp.value!=="Custom")mp.value="Custom";}
        }
      }
      persistServiceNodeUIState(node);scheduleServiceNodePanelSync(node);redrawNode(node);return result;
    };
  }
  for (const [textName, selectorName] of [["system_prompt","system_prompt_preset"],["prompt","prompt_preset"]]) {
    const w=nodeWidget(node,textName);if(!w)continue;const old=w.callback;
    w.callback=function(value,...args){const result=old?.call(this,value,...args);if(!node.__localLLMApplyingPreset){const p=nodeWidget(node,selectorName);if(p&&p.value!=="Custom")p.value="Custom";}persistServiceNodeUIState(node);scheduleServiceNodePanelSync(node);redrawNode(node);return result;};
  }
}
...
const SETTINGS_MODEL_FIELDS = [
  ["model","Model"],["vision_model","Vision / mmproj"],["model_preset","Model Preset"],
  ["thinking_mode","Thinking Mode"],["reasoning_effort","Reasoning Effort"],
];
const SETTINGS_MEMORY_NUMERIC_FIELDS = [
  ["context_size","Context Size"],["gpu_layers","GPU Layers"],["prompt_batch_size","Prompt Batch"],
  ["memory_batch_size","Micro Batch"],["ngram_pred_tokens","N-gram Draft Tokens"],["ngram_size","N-gram Size"],
  ["ngram_min_hits","N-gram Min Hits"],["ngram_max_entries_per_key","N-gram Entries / Key"],
  ["ngram_sync_check_tokens","N-gram Sync Tokens"],["mtp_draft_tokens","MTP Draft Tokens"],["mtp_p_min","MTP P-Min"],
];
const SETTINGS_MEMORY_SELECT_FIELDS = [
  ["kv_cache_k","KV Cache K"],["kv_cache_v","KV Cache V"],["kv_cache_location","KV Location"],
  ["prompt_cache_mode","Prompt Cache"],["speculative_mode","Speculative"],["ngram_mode","N-gram Mode"],
  ["main_gpu","Main GPU"],["split_mode","Split Mode"],["vram_policy","VRAM Policy"],
];
const SETTINGS_MEMORY_CHECK_FIELDS = [
  ["flash_attention","Flash Attention"],["use_mmap","Use mmap"],["use_mlock","Use mlock"],
];
const SETTINGS_ALL_PRESET_OWNED = new Set([
  ...SETTINGS_MODEL_FIELDS.map(x=>x[0]),...SERVICE_NUMERIC_FIELDS.map(x=>x[0]),
  ...SETTINGS_MEMORY_NUMERIC_FIELDS.map(x=>x[0]),...SETTINGS_MEMORY_SELECT_FIELDS.map(x=>x[0]),
  ...SETTINGS_MEMORY_CHECK_FIELDS.map(x=>x[0]),"preserve_thinking","tensor_split",
]);
...
function servicePanelHasScrollableOverflow(el) {
...
function servicePanelWrapScrollableControl(scrollEl) {
  const shell = servicePanelElement("div", "llmn-wheel-capture");
  shell.tabIndex = -1;
  shell.__localLLMWheelScrollElement = scrollEl;
  shell.appendChild(scrollEl);

  const syncCapture = () => {
    if (servicePanelHasScrollableOverflow(scrollEl)) shell.dataset.captureWheel = "true";
    else shell.removeAttribute("data-capture-wheel");
  };
  scrollEl.__localLLMSyncWheelCapture = syncCapture;

  // A scrollable field owns wheel input whenever the pointer is over it. Native
  // scrolling does not require focus, so do not focus proxy elements on hover.
...
function servicePanelShouldCaptureWheel(event, root) {
  const target = event.target instanceof Element ? event.target : null;
  const capture = target?.closest?.(".llmn-wheel-capture");
  if (!capture || !root.contains(capture)) return false;
  const scrollEl = capture.__localLLMWheelScrollElement;
  const scrollable = !!scrollEl && servicePanelHasScrollableOverflow(scrollEl);
  if (scrollable) capture.dataset.captureWheel = "true";
  else capture.removeAttribute("data-capture-wheel");
  return scrollable;
}

// Classic DOM widgets sit above LiteGraph's canvas as separate DOM hit targets,
// so an ordinary bubbling wheel event cannot reach the canvas. Forward eligible
// wheel input directly to ComfyUI's real canvas, mirroring the frontend's own
// useCanvasInteractions forwarding behavior.
function servicePanelForwardWheelToCanvas(event) {
...
function servicePanelHandleWheel(event, root) {
  // Scrollable DOM fields keep wheel input; only non-scrollable overlay regions
  // are forwarded to the ComfyUI workspace canvas.
  if (servicePanelShouldCaptureWheel(event, root)) {
    event.stopPropagation();
    return;
  }
  servicePanelForwardWheelToCanvas(event);
}
...
const SERVICE_INTEGER_FIELDS = new Set(["top_k","max_tokens","vision_max_images","vision_max_frames","vision_max_edge","seed","context_size","gpu_layers","prompt_batch_size","memory_batch_size","ngram_pred_tokens","ngram_size","ngram_min_hits","ngram_max_entries_per_key","ngram_sync_check_tokens","mtp_draft_tokens"]);
...
function hideNativeWidgetsForServicePanel(node) {
  ...
  const names = [
    "settings_preset","system_prompt_preset","system_prompt","prompt_preset","prompt","sampling_mode",
    ...REQUEST_PRESET_FIELDS,"vision_max_images","vision_max_frames","vision_max_edge",
    ...SETTINGS_MODEL_FIELDS.map(x=>x[0]),...SETTINGS_MEMORY_NUMERIC_FIELDS.map(x=>x[0]),
    ...SETTINGS_MEMORY_SELECT_FIELDS.map(x=>x[0]),...SETTINGS_MEMORY_CHECK_FIELDS.map(x=>x[0]),
    "preserve_thinking","tensor_split","seed",
  ];
...
function addServicePanelSelectField(node, content, controls, name, labelText) {
  const w=nodeWidget(node,name);if(!w)return;
  const field=servicePanelElement("div","llmn-field");field.appendChild(servicePanelFieldLabel(labelText));
  const select=servicePanelElement("select","llmn-select");setServicePanelSelectOptions(select,servicePanelComboValues(w),w.value);
  select.addEventListener("change",()=>setServicePanelNativeValue(node,name,select.value));field.appendChild(select);content.appendChild(field);controls[name]=select;
}
function addServicePanelCheckboxField(node, content, controls, name, labelText) {
  const w=nodeWidget(node,name);if(!w)return;
  const field=servicePanelElement("div","llmn-field");field.appendChild(servicePanelFieldLabel(labelText));
  const wrap=servicePanelElement("label");Object.assign(wrap.style,{display:"flex",alignItems:"center",minHeight:"30px",gap:"8px"});
  const input=servicePanelElement("input");input.type="checkbox";input.checked=!!w.value;input.addEventListener("change",()=>setServicePanelNativeValue(node,name,!!input.checked));wrap.appendChild(input);field.appendChild(wrap);content.appendChild(field);controls[name]=input;
}
function addServicePanelTextField(node, content, controls, name, labelText) {
  const w=nodeWidget(node,name);if(!w)return;const field=servicePanelElement("div","llmn-field");field.appendChild(servicePanelFieldLabel(labelText));
  const input=servicePanelElement("input","llmn-input");input.type="text";input.value=String(w.value??"");input.addEventListener("change",()=>setServicePanelNativeValue(node,name,input.value));field.appendChild(input);content.appendChild(field);controls[name]=input;
}
function addSettingsPresetLoadField(node, content, controls){
  const w=nodeWidget(node,"settings_preset");if(!w)return;const field=servicePanelElement("div","llmn-field");field.appendChild(servicePanelFieldLabel("Settings Preset"));
  const select=servicePanelElement("select","llmn-select");select.addEventListener("change",()=>setServicePanelNativeValue(node,"settings_preset",select.value));field.appendChild(select);content.appendChild(field);controls.settings_preset_select=select;
  const note=servicePanelElement("div","llmn-badge");note.textContent="Load-only here • save/delete presets in the Local LLM Presets tab";content.appendChild(note);controls.settingsPresetNote=note;
}
...
function servicePanelIntrinsicLabelWidth(label) {
  if(!label) return 0;
  try {
    const style=getComputedStyle(label);
    const canvas=servicePanelIntrinsicLabelWidth.__canvas || (servicePanelIntrinsicLabelWidth.__canvas=document.createElement("canvas"));
    const ctx=canvas.getContext("2d");
    if(ctx){
      ctx.font=style.font || `${style.fontWeight || "600"} ${style.fontSize || "13px"} ${style.fontFamily || "sans-serif"}`;
      const text=String(label.textContent || "");
      const measured=ctx.measureText(text).width;
      const letterSpacing=parseFloat(style.letterSpacing);
      const spacing=Number.isFinite(letterSpacing) && text.length>1 ? letterSpacing*(text.length-1) : 0;
      return measured+spacing;
    }
  } catch(_) {}
  return Number(label.scrollWidth)||0;
}

function syncServicePanelLabelWidths(node) {
  const root=node?.__localLLMPanelRoot;
  if(!root) return;

  // Only numeric rows share a label column. Other field rows size their label
  // intrinsically so one long label cannot squeeze every unrelated control.
  const numberLabels=[...root.querySelectorAll(".llmn-number-field > .llmn-field-label")];
  let numberMax=0;
  for(const label of numberLabels) numberMax=Math.max(numberMax,servicePanelIntrinsicLabelWidth(label));
  if(numberMax>0) root.style.setProperty("--llmn-number-label-width",`${Math.ceil(numberMax+8)}px`);
}
...
function addServicePanelTextarea(node, content, controls, name, placeholder) {
  ...
  content.appendChild(servicePanelWrapScrollableControl(area));
  controls[name]=area;
}
...
function createServiceNodeDomPanel(node) {
  ...
  // Forward non-scrollable wheel input to the workspace canvas. Focused,
  // overflowing textareas keep native wheel scrolling instead.
  root.addEventListener("wheel",(e)=>servicePanelHandleWheel(e,root),{passive:false});
  ...
  css.textContent=`
    ...
    .local-llm-service-node-panel .llmn-wheel-capture{display:block;width:100%;min-width:0;max-width:100%;outline:none}
    ...
    .local-llm-service-node-panel .llmn-field{display:grid;grid-template-columns:max-content minmax(0,1fr);gap:8px;align-items:center;width:100%}
    .local-llm-service-node-panel .llmn-number-field{grid-template-columns:var(--llmn-number-label-width,152px) minmax(0,1fr)}
    .local-llm-service-node-panel .llmn-field-label{font-weight:600;opacity:.9;min-width:0;white-space:nowrap}
    ...
    .local-llm-service-node-panel .llmn-mini-label{display:block;font-size:11px;font-weight:600;opacity:.75;margin:0 0 3px 2px;white-space:nowrap}
  ...
  const isSettings=node?.comfyClass==="LocalLLMSettings";
  if(!isSettings && nodeWidget(node,"system_prompt_preset")){
  ...
  if(isSettings){
    addSettingsPresetLoadField(node,content,controls);
    // Model, vision projector, and model preset are owned by the selected
    // Complete Settings Preset. Keep them as hidden native widgets so preset
    // loading and workflow serialization remain backward compatible, but do
    // not expose duplicate model selectors in the Settings node UI.
    addServicePanelSelectField(node,content,controls,"thinking_mode","Thinking Mode");
    addServicePanelSelectField(node,content,controls,"reasoning_effort","Reasoning Effort");
    addServicePanelCheckboxField(node,content,controls,"preserve_thinking","Preserve Thinking History");
  }
  ...
  // Seed is request-local and appears only on request nodes such as Generate.
  // Local LLM Settings deliberately has no seed widget.
  if(nodeWidget(node,"seed")){
  ...
  requestAnimationFrame(()=>syncServicePanelLabelWidths(node));
  ...
  if(typeof ResizeObserver!=="undefined"){
    const ro=new ResizeObserver(()=>{syncServicePanelLabelWidths(node);scheduleServiceNodePanelHeight(node);}); ro.observe(content); node.__localLLMPanelResizeObserver=ro;
    const tro=new ResizeObserver(()=>{captureServiceTextareaHeights(node);controls.system_prompt?.__localLLMSyncWheelCapture?.();controls.prompt?.__localLLMSyncWheelCapture?.();});
...
function syncServiceNodeDomPanel(node) {
  ...
  if(!c) return;
  const syncText=(name)=>{if(c[name]){const v=String(nodeWidget(node,name)?.value ?? "");if(c[name].value!==v)c[name].value=v;c[name].__localLLMSyncWheelCapture?.();}};
  ...
  for(const [name,kind] of presetSpecs){
    ...
    const values=name==="settings_preset" ? (node.__localLLMPresetCatalog?.settings?.names||servicePanelComboValues(w)) : servicePanelComboValues(w);
    setServicePanelSelectOptions(select,values,w.value);
  ...
  for(const [name] of [...SERVICE_NUMERIC_FIELDS,...SETTINGS_MEMORY_NUMERIC_FIELDS]){
    const input=c[name], w=nodeWidget(node,name); if(!input || !w) continue;
    syncServicePanelNumberControl(input,w,name);
  }
  for(const [name] of [...SETTINGS_MODEL_FIELDS,...SETTINGS_MEMORY_SELECT_FIELDS]){
    const el=c[name],w=nodeWidget(node,name);if(!el||!w)continue;setServicePanelSelectOptions(el,servicePanelComboValues(w),w.value);
  }
  for(const [name] of SETTINGS_MEMORY_CHECK_FIELDS){const el=c[name],w=nodeWidget(node,name);if(el&&w)el.checked=!!w.value;}
  if(c.preserve_thinking){const w=nodeWidget(node,"preserve_thinking");if(w)c.preserve_thinking.checked=!!w.value;}
  if(c.tensor_split){const w=nodeWidget(node,"tensor_split");if(w&&c.tensor_split.value!==String(w.value??""))c.tensor_split.value=String(w.value??"");}
  ...
  const requestControlNames=[...SERVICE_NUMERIC_FIELDS.map(([n])=>n)];
  for(const name of requestControlNames) if(c[name]) setServicePanelNumberDisabled(c[name],settingsConnected);
  // Seed is always owned by the request node, never by Local LLM Settings.
  if(c.seed) setServicePanelNumberDisabled(c.seed,false);
  if(c.seedControl) c.seedControl.disabled=false;
  c.seedWrap?.classList.toggle("llmn-disabled",false);
...
function wrapServiceNodePanelSyncCallbacks(node) {
  ...
  const names=["settings_preset","system_prompt_preset","system_prompt","prompt_preset","prompt","sampling_mode",...REQUEST_PRESET_FIELDS,"vision_max_images","vision_max_frames","vision_max_edge",...SETTINGS_MODEL_FIELDS.map(x=>x[0]),...SETTINGS_MEMORY_NUMERIC_FIELDS.map(x=>x[0]),...SETTINGS_MEMORY_SELECT_FIELDS.map(x=>x[0]),...SETTINGS_MEMORY_CHECK_FIELDS.map(x=>x[0]),"preserve_thinking","tensor_split","seed"];
...
async function initializeServiceGenerateNode(node, loaded=false, serializedSize=null) {
  ...
  try{
    ...
    const systemSel=nodeWidget(node,"system_prompt_preset");
    const promptSel=nodeWidget(node,"prompt_preset");
    if(settingsSel && settingsSel.value!=="Custom") await applySettingsPresetSelection(node,settingsSel.value,data);
    if(sampler && sampler.value!=="Custom" && node?.comfyClass!=="LocalLLMSettings") await applyPresetSelection(node,"sampler",sampler.value,data);
...
app.registerExtension({
  ...
  nodeCreated(node) {
    if (node?.comfyClass === "LocalLLMGenerate" || node?.comfyClass === "LocalLLMSettings") {
