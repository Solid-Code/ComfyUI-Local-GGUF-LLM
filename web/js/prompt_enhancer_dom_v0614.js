...
const FRONTEND_VERSION = "0.6.13-alpha";
...
async function runTextOnlyEnhancementOnce(node, prompt, enhancementText, seed) {
...
async function runMediaEnhancementOnce(node, prompt, enhancementText, seed) {
  let token = "";
  try {
    const controller = new AbortController();
    node.__promptEnhancerRequestController = controller;
  ...
  try {
    for (let index = 0; index < count; index += 1) {
      ...
      if (usesPartialExecution) {
        data = await runMediaEnhancementOnce(node, prompt, enhancementText, seed);
      } else {
        data = await runTextOnlyEnhancementOnce(node, prompt, enhancementText, seed);
        finishSeedControlCycle(node);
      }
...
function panelIsTextEditingElement(el) {
...
function panelWrapScrollableControl(scrollEl) {
  ...
  shell.addEventListener("pointerenter", (event) => {
    ...
    if (active && active !== document.body && active !== document.documentElement &&
        active !== shell && !shell.contains(active) && panelIsTextEditingElement(active)) return;
...
function panelHandleWheel(event, root) {
  // A focused, genuinely overflowing textarea owns normal wheel scrolling.
  // Ctrl/Meta bypasses this via panelShouldCaptureWheel() and is always sent to
  // the workspace, matching ComfyUI's canvas-gesture behavior.
