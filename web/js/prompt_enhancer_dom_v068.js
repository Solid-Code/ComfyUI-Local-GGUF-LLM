...
const EXTENSION_NAME = "LocalLLM.CreatePrompt";
const FRONTEND_VERSION = "0.6.7-alpha";
...
function pushArrayUndo(node) {
  ...
  node.__promptEnhancerArrayUndo.push(arraySnapshot(node));
  if (node.__promptEnhancerArrayUndo.length > 20) node.__promptEnhancerArrayUndo.shift();
}
...
function undoPromptArray(node) {
  if (!node || node.__promptEnhancerBusy) return;
  const stack = node.__promptEnhancerArrayUndo || [];
  if (!stack.length) {
...
function beginSeedControlCycle(node) {
  ...
  // Local LLM Prompt Enhancer's own seed remains the fallback.
...
async function runMediaEnhancement(node, prompt, enhancementText, seed) {
  ...
  try {
    ...
    if (!token) throw new Error("Local LLM Prompt Enhancer could not arm the request.");
    ...
    if (!jobId) {
      const message = queued?.error?.message || "ComfyUI rejected the Local LLM Prompt Enhancer partial execution.";
      throw new Error(message);
    }
...
function createPromptEnhancerDomPanel(node) {
  ...
  css.textContent = `
    ...
    .local-llm-prompt-enhancer-panel .pe-seed {
    ...
    .local-llm-prompt-enhancer-panel .pe-seed:focus {
    ...
    .local-llm-prompt-enhancer-panel .pe-seed { min-height: 38px; padding: 5px 8px; }
    ...
    .local-llm-prompt-enhancer-panel .pe-history-index {
      display: inline-flex; align-items: center; justify-content: center; gap: 3px;
      min-height: 38px; padding: 0 6px; border: 1px solid rgba(127,127,127,.42);
      border-radius: 6px; background: rgba(127,127,127,.12); font-variant-numeric: tabular-nums;
    }
    .local-llm-prompt-enhancer-panel .pe-index-input {
      width: 46px; min-width: 0; border: 0; outline: 0; background: transparent; color: inherit;
      font: inherit; font-weight: 600; text-align: right; padding: 0; appearance: textfield;
    }
    .local-llm-prompt-enhancer-panel .pe-index-input::-webkit-inner-spin-button,
    .local-llm-prompt-enhancer-panel .pe-index-input::-webkit-outer-spin-button { appearance: none; margin: 0; }
    .local-llm-prompt-enhancer-panel .pe-index-total { opacity: .72; white-space: nowrap; }
    ...
    @media (max-width: 520px) {
      ...
      .local-llm-prompt-enhancer-panel .pe-history-index { min-height: 42px; }
      ...
      .local-llm-prompt-enhancer-panel .pe-select,
      .local-llm-prompt-enhancer-panel .pe-seed { min-height: 42px; }
  ...
  controls.prev = panelButton("←", "Previous enhanced prompt", () => cycleStoredPrompt(node, -1));
  const historyIndex = panelElement("div", "pe-history-index");
  controls.historyIndex = panelElement("input", "pe-index-input");
  controls.historyIndex.type = "number";
  controls.historyIndex.min = "1";
  controls.historyIndex.step = "1";
  controls.historyIndex.inputMode = "numeric";
  controls.historyIndex.title = "Active enhanced-prompt index";
  controls.historyTotal = panelElement("span", "pe-index-total");
  controls.historyTotal.textContent = "/ 0";
  const commitHistoryIndex = () => selectStoredPromptIndex(node, controls.historyIndex.value);
  controls.historyIndex.addEventListener("change", commitHistoryIndex);
  controls.historyIndex.addEventListener("keydown", (event) => {
    if (event.key !== "Enter") return;
    event.preventDefault();
    commitHistoryIndex();
    controls.historyIndex.blur();
  });
  historyIndex.append(controls.historyIndex, controls.historyTotal);
  controls.next = panelButton("→", "Next enhanced prompt", () => cycleStoredPrompt(node, 1));
  controls.deletePrompt = panelButton("×", "Delete active enhanced prompt", () => deleteActivePrompt(node));
  controls.undoArray = panelButton("Undo", "Undo last enhanced-prompt array change", () => undoPromptArray(node));
  controls.clear = panelButton("Clear All", "Clear all enhanced prompts", () => clearPromptArray(node));
  historyRow.append(controls.prev, historyIndex, controls.next, controls.deletePrompt, controls.undoArray, controls.clear);
  ...
  controls.seed = panelElement("input", "pe-seed");
  controls.seed.type = "text";
  controls.seed.inputMode = "numeric";
  controls.seed.autocomplete = "off";
  controls.seed.addEventListener("change", () => {
    const raw = Number(controls.seed.value);
    const value = Number.isFinite(raw) ? Math.max(0, Math.trunc(raw)) : 0;
    setPanelNativeValue(node, "seed", value);
  });
  seedValueBox.append(seedValueLabel, controls.seed);
...
function syncDomPanel(node) {
  ...
  const seedWidget = widget(node, "seed");
  c.seed.value = String(seedWidget?.value ?? 0);
  ...
  const settingsConnected = hasConnectedSettings(node);
  c.seed.disabled = settingsConnected;
  ...
  const index = promptHistoryIndex(node, history);
  c.historyIndex.min = history.length ? "1" : "0";
  c.historyIndex.max = String(history.length || 0);
  c.historyIndex.value = history.length ? String(index + 1) : "0";
  c.historyTotal.textContent = `/ ${history.length}`;
  ...
  c.prev.disabled = busy || !hasHistory;
  c.historyIndex.disabled = busy || !hasHistory;
  c.next.disabled = busy || !hasHistory;
...
function promptEnhancerMenuItems(node) {
  ...
  return [
    null,
    {
      content: enhancing ? "Local LLM Prompt Enhancer: Cancel Enhancement" : "Local LLM Prompt Enhancer: Enhance Prompt",
      disabled: busy && !enhancing,
      callback: () => toggleEnhance(node),
    },
    {
      content: "Local LLM Prompt Enhancer: Use Enhanced as Prompt (↑)",
      disabled: disabledWhileBusy || !hasHistory,
      callback: () => useEnhancedAsPrompt(node),
    },
    {
      content: "Local LLM Prompt Enhancer: Undo Prompt Replacement",
