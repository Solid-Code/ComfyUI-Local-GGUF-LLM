...
function refreshSettingsPresetTab(preferred=null){
  ...
  const names=(section.names||["Custom"]).filter(x=>x!=="Custom");
  select.innerHTML=selectOptions(names.length?names:["— No saved presets —"],names.includes(previous)?previous:(names[0]||"— No saved presets —"));
  select.disabled=!names.length;
  const current=select.disabled?null:select.value;
  const summary=modal.querySelector("#llm-settings-preset-summary");
  if(summary)summary.textContent=current?settingsPresetSummary(section.presets?.[current]):"No complete settings presets have been saved yet.";
  const del=modal.querySelector('[data-action="preset-delete"]');if(del)del.disabled=!current||!(section.deletable_names||[]).includes(current);
  const load=modal.querySelector('[data-action="preset-load"]');if(load)load.disabled=!current;
...
async function loadSelectedSettingsPreset(){
  ...
  if(!settings)return;await applyCompleteSettingsToModal(settings);const n=modal?.querySelector("#llm-save-note");if(n)n.textContent=`Loaded preset: ${name} — save configuration to make it active.`;
}
...
function servicePanelIsTextEditingElement(el) {
  return el instanceof HTMLElement && (
    el.matches?.("input, textarea, select, [contenteditable='true']") || el.isContentEditable
  );
}

// Current ComfyUI Nodes 2.0 lets a focused DOM subtree opt out of canvas wheel
// handling with data-capture-wheel="true". Use a focus proxy around only
// genuinely scrollable controls: ordinary DOM keeps the workspace wheel/zoom,
// while an overflowing textarea can still scroll when hovered or focused.
function servicePanelWrapScrollableControl(scrollEl) {
  ...
  shell.addEventListener("pointerenter", (event) => {
    syncCapture();
    // Ctrl/Meta + wheel belongs to ComfyUI zoom in Standard navigation mode.
    if (event.ctrlKey || event.metaKey) {
      shell.removeAttribute("data-capture-wheel");
      return;
    }
    if (shell.dataset.captureWheel !== "true") return;
    const active = document.activeElement;
    // Do not steal an active text-editing session elsewhere just because the
    // pointer crossed this textarea. The user can click this textarea to focus it.
    if (active && active !== document.body && active !== document.documentElement &&
        active !== shell && !shell.contains(active) && servicePanelIsTextEditingElement(active)) return;
    if (!shell.contains(active)) {
      try { shell.focus({ preventScroll: true }); } catch (_) { try { shell.focus(); } catch (_) {} }
    }
  });
  shell.addEventListener("pointerleave", () => {
    if (document.activeElement === shell) { try { shell.blur(); } catch (_) {} }
  });
  shell.addEventListener("keydown", (event) => {
    if (event.key === "Control" || event.key === "Meta") shell.removeAttribute("data-capture-wheel");
  });
  shell.addEventListener("keyup", (event) => {
    if (event.key === "Control" || event.key === "Meta") syncCapture();
  });
...
function servicePanelShouldCaptureWheel(event, root) {
  ...
  const capture = target?.closest?.('[data-capture-wheel="true"]');
  return !!capture && root.contains(capture) && capture.contains(document.activeElement);
}
